...
if(fig2):
    h_fig2 = np.arange(0,14400,100).tolist()
...
if(tab1):
    ...
    for h in h_list:
        ...
        for W_kg in W_list_kg:     
            ...
            df[(W_kg)] = X_pista
        Height_df_takeoff.append(df)


    print(Height_df_takeoff[0].to_latex(),'\n\n')
    print(Height_df_takeoff[1].to_latex(),'\n\n')
    print(Height_df_takeoff[2].to_latex())
...
if(tab2):
    ...
    h_list = np.arange(0, 3000, 1000).tolist()
    Temp_list = np.arange(-20, 45, 5).tolist()
    W_list_kg = np.linspace((12000), (17000), 6, endpoint=True)
    
    initial_data_la={'Temp':Temp_list, 'Landing Weight':W_list_kg}
    Height_df_landing=[]
    for h in h_list:
        ...
        for W_kg in W_list_kg:     
            ...
            for Temp in Temp_list:               
                ...
                #Rotação
                x_RLa = decolagem.rotation(V_S)
                
                # desaceleração de solo
                x_GLa = pouso.landing_roll(V_S)
                                
                #Distancia de pista
                x_pista_la = (x_f+ x_ap+ x_RLa+ x_GLa)
    ...
    print(Height_df_landing[2].to_latex())